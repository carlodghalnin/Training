/*----------------------------------------------------
Advanced SQL puzzle DDL and questions from Scott 
Peters. 
...
/*----------------------------------------------------
#1 Shopping Carts tables
Perform a side-by-side audit of both carts.
*/----------------------------------------------------
...
--Puzzle 1 Solution
SELECT A.Item AS 'Cart 1', B.Item AS 'Cart 2'
	FROM #Cart1 A FULL JOIN #Cart2 B 
	ON A.Item = B.Item;

/*----------------------------------------------------
#2 Managers and Employees tables
Determine depth employees have from president.
*/----------------------------------------------------
...
--Puzzle 2 solution: recursive CTE
WITH levels(EmployeeID, ManagerID, JobTitle, Salary, 
Depth) AS
(
	SELECT EmployeeID, ManagerID, JobTitle, Salary, 
	0 AS Depth FROM #Employees WHERE ManagerID IS NULL
	UNION ALL
	SELECT e.EmployeeID, e.ManagerID, e.JobTitle, e.Salary, 
	Depth + 1 FROM #Employees AS e
	INNER JOIN levels AS l ON l.EmployeeID = e.ManagerID
)
SELECT * FROM levels;
